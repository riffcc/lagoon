//! SPIRAL-scoped peer address gossip coordinator.
//!
//! Mirrors [`super::connection_gossip::ConnectionGossip`] but for peer address
//! records instead of connection snapshots. Coordinates WHICH SPIRAL neighbors
//! to gossip with and WHEN to ensure all nodes eventually learn about all peer
//! connection addresses — even if they missed initial discovery.
//!
//! ## Why this exists
//!
//! The existing `MeshMessage::Peers` re-broadcast is one-shot at initial
//! discovery time. If node A learns about node X AFTER establishing its relay
//! to node B, node B never hears about X unless X later connects to B directly.
//! SPORE-based address gossip closes this gap: any new address in the store
//! is continuously propagated to SPIRAL neighbors until convergence.

use std::collections::{HashMap, HashSet};

use citadel_spore::{Spore, U256};
use serde::{Deserialize, Serialize};

/// Wire message for peer address gossip between SPIRAL neighbors.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SyncMessage {
    /// "Here's my SPORE state — send me what I'm missing"
    HaveList { spore_bytes: Vec<u8> },
    /// "Here are the address records you're missing"
    RecordDelta { entries: Vec<Vec<u8>> },
}

/// Action for federation.rs to execute.
#[derive(Debug, Clone)]
pub enum SyncAction {
    /// Send a HaveList to this neighbor to initiate reconciliation.
    SendHaveList {
        neighbor_peer_id: String,
        message: SyncMessage,
    },
    /// Send address record deltas to this neighbor.
    SendRecordDelta {
        neighbor_peer_id: String,
        message: SyncMessage,
    },
}

/// SPIRAL-scoped peer address gossip coordinator.
///
/// Tracks which SPIRAL neighbors exist and enforces a minimum sync interval.
/// Routing (relay lookup by peer_id) is handled by the federation layer.
#[derive(Debug)]
pub struct PeerAddrGossip {
    spiral_neighbors: HashSet<String>,
    last_sync: HashMap<String, i64>,
    sync_interval_ms: i64,
}

impl PeerAddrGossip {
    /// Create a new peer address gossip coordinator.
    pub fn new(sync_interval_ms: i64) -> Self {
        Self {
            spiral_neighbors: HashSet::new(),
            last_sync: HashMap::new(),
            sync_interval_ms,
        }
    }

    /// Replace the SPIRAL neighbor set (called when topology changes).
    pub fn set_spiral_neighbors(&mut self, neighbors: HashSet<String>) {
        self.last_sync.retain(|peer, _| neighbors.contains(peer));
        self.spiral_neighbors = neighbors;
    }

    /// Called when the peer address store changes (new peer learned or updated).
    ///
    /// Returns [`SyncAction`]s for SPIRAL neighbors that are due for sync.
    /// The federation layer filters by relay availability.
    pub fn on_store_updated(
        &mut self,
        now_ms: i64,
        our_spore_bytes: &[u8],
    ) -> Vec<SyncAction> {
        let due = self.neighbors_needing_sync(now_ms);
        let mut actions = Vec::with_capacity(due.len());

        for peer_id in due {
            actions.push(SyncAction::SendHaveList {
                neighbor_peer_id: peer_id.clone(),
                message: SyncMessage::HaveList {
                    spore_bytes: our_spore_bytes.to_vec(),
                },
            });
            self.mark_synced(&peer_id, now_ms);
        }

        actions
    }

    /// Called when we receive a `HaveList` from a neighbor.
    ///
    /// Returns a `SendRecordDelta` action if we have records they're missing.
    pub fn on_have_list_received(
        &self,
        from_peer_id: &str,
        their_spore_bytes: &[u8],
        our_spore: &Spore,
        our_data: &[(Vec<u8>, [u8; 32])],
    ) -> Option<SyncAction> {
        let their_spore: Spore = match bincode::deserialize(their_spore_bytes) {
            Ok(s) => s,
            Err(_) => return None,
        };

        let diff = our_spore.subtract(&their_spore);
        if diff.is_empty() {
            return None;
        }

        let entries: Vec<Vec<u8>> = our_data
            .iter()
            .filter(|(_, content_id)| {
                let u = U256::from_be_bytes(content_id);
                diff.covers(&u)
            })
            .map(|(bytes, _)| bytes.clone())
            .collect();

        if entries.is_empty() {
            return None;
        }

        Some(SyncAction::SendRecordDelta {
            neighbor_peer_id: from_peer_id.to_owned(),
            message: SyncMessage::RecordDelta { entries },
        })
    }

    /// Return peers due for a sync round.
    fn neighbors_needing_sync(&self, now_ms: i64) -> Vec<String> {
        self.spiral_neighbors
            .iter()
            .filter(|peer| match self.last_sync.get(*peer) {
                Some(&last) => now_ms - last >= self.sync_interval_ms,
                None => true,
            })
            .cloned()
            .collect()
    }

    /// Mark a neighbor as synced at the given timestamp.
    fn mark_synced(&mut self, peer_id: &str, now_ms: i64) {
        if self.spiral_neighbors.contains(peer_id) {
            self.last_sync.insert(peer_id.to_owned(), now_ms);
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use citadel_spore::{Range256, Spore, U256};

    fn make_gossip() -> PeerAddrGossip {
        PeerAddrGossip::new(5000)
    }

    fn neighbors_set(names: &[&str]) -> HashSet<String> {
        names.iter().map(|s| (*s).to_owned()).collect()
    }

    #[test]
    fn test_new_empty() {
        let g = make_gossip();
        assert!(g.spiral_neighbors.is_empty());
        assert!(g.last_sync.is_empty());
    }

    #[test]
    fn test_set_spiral_neighbors() {
        let mut g = make_gossip();
        g.set_spiral_neighbors(neighbors_set(&["a", "b"]));
        assert_eq!(g.spiral_neighbors.len(), 2);

        g.set_spiral_neighbors(neighbors_set(&["x"]));
        assert_eq!(g.spiral_neighbors.len(), 1);
        assert!(g.spiral_neighbors.contains("x"));
    }

    #[test]
    fn test_on_store_updated_sends_to_due() {
        let mut g = make_gossip();
        g.set_spiral_neighbors(neighbors_set(&["a", "b"]));

        let actions = g.on_store_updated(10_000, b"fake-spore");
        assert_eq!(actions.len(), 2);
    }

    #[test]
    fn test_on_store_updated_skips_recent() {
        let mut g = make_gossip();
        g.set_spiral_neighbors(neighbors_set(&["a"]));

        let _ = g.on_store_updated(0, b"s");
        let actions = g.on_store_updated(3000, b"s");
        assert!(actions.is_empty());

        let actions = g.on_store_updated(5000, b"s");
        assert_eq!(actions.len(), 1);
    }

    #[test]
    fn test_on_have_list_with_diff() {
        let g = make_gossip();

        let content_id: [u8; 32] = blake3::hash(b"test peer record").into();
        let u = U256::from_be_bytes(&content_id);
        let next = u.checked_add(&U256::from_u64(1)).unwrap_or(U256::MAX);
        let our_spore = Spore::from_range(Range256::new(u, next));

        let their_spore = Spore::empty();
        let their_bytes = bincode::serialize(&their_spore).unwrap();

        let our_data: Vec<(Vec<u8>, [u8; 32])> =
            vec![(b"record-data".to_vec(), content_id)];

        let action = g
            .on_have_list_received("peer_x", &their_bytes, &our_spore, &our_data)
            .expect("should produce a RecordDelta");

        match action {
            SyncAction::SendRecordDelta {
                neighbor_peer_id,
                message,
            } => {
                assert_eq!(neighbor_peer_id, "peer_x");
                match message {
                    SyncMessage::RecordDelta { entries } => {
                        assert_eq!(entries.len(), 1);
                    }
                    _ => panic!("expected RecordDelta"),
                }
            }
            _ => panic!("expected SendRecordDelta"),
        }
    }

    #[test]
    fn test_on_have_list_synced() {
        let g = make_gossip();

        let content_id: [u8; 32] = blake3::hash(b"synced data").into();
        let u = U256::from_be_bytes(&content_id);
        let next = u.checked_add(&U256::from_u64(1)).unwrap_or(U256::MAX);
        let our_spore = Spore::from_range(Range256::new(u, next));

        let their_bytes = bincode::serialize(&our_spore).unwrap();
        let our_data: Vec<(Vec<u8>, [u8; 32])> = vec![(b"entry".to_vec(), content_id)];

        let result = g.on_have_list_received("peer_x", &their_bytes, &our_spore, &our_data);
        assert!(result.is_none());
    }

    #[test]
    fn test_set_neighbors_clears_stale_sync() {
        let mut g = make_gossip();
        g.set_spiral_neighbors(neighbors_set(&["a", "b"]));
        let _ = g.on_store_updated(0, b"s");

        g.set_spiral_neighbors(neighbors_set(&["a"]));
        assert!(!g.last_sync.contains_key("b"));
    }
}
